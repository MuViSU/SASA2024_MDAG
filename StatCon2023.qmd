...
::: {.column width="25%"}
![Prof Sugnet Lubbe (SU)](photos\SL.jpg) ![Prof. Niël le Roux](photos\NJLR.jpg)
:::
...
::: {layout="[[1],[3,1]]"}
![](figures\Scatterplot%201.png){height="500"} ![](figures\Table%201.png){height="500"}
:::
...
::: {.column width="40%"}
::: fragment
![](figures\points_in2d.png){fig-align="center"}
:::
:::
:::
...
![](figures\points_with_vectors.png){fig-align="center"}
...
::: {layout-ncol="2"}
![](figures\GlobalOptions.jpg){width="10%"}

![](figures\GlobalOptionsPipe.jpg){width="25%"}
:::
:::
...
-   Use the preferred biplot: [PCA()]{style="font-family: 'Noto Sans Mono', monospace"} or [CVA()]{style="font-family: 'Noto Sans Mono', monospace"}, available in current version.
    ...
    -   Add [alpha.bags()]{style="font-family: 'Noto Sans Mono', monospace"} or [ellipses()]{style="font-family: 'Noto Sans Mono', monospace"}.
...
Use pipe ( \|\> ) between each subsequent function. The order of [samples(), axes(), alpha.bags(), ellipses() and legend.type()]{style="font-family: 'Noto Sans Mono', monospace"} can be interchanged.
...
biplot(iris[,1:4], group.aes = iris[,5]) |> PCA() |> axes(col = "black", tick.col = 
  "black", tick.size = 1.2, ticks = 5, tick.label.side = c("below","above","above","below"), lwd = 1.2) |> plot()
```
...
#changing axes names
biplot(iris[,1:4], group.aes = iris[,5]) |> PCA() |> axes(col = "black", tick.col = 
  "black", tick.size = 1.2, lwd = 1.2, ticks = 7, tick.label.side = c("below","above","above","below"), ax.names = c("A", "B", "","D"), label.dir = "Hor") |> plot()
```
...
biplot(iris[,1:4], group.aes = iris[,5]) |> PCA() |> axes(col = "black", tick.col = 
  "black", tick.label.side = c("above","below","below","above"),  orthogx = 2) |> plot()
```
...
biplot(iris[,1:4], group.aes = iris[,5]) |> PCA() |> axes(col = "black", tick.col = 
  "black", tick.label.side = c("above","below","below","above"), orthogy = -2) |> plot()
```
...
## [ellipses()]{style="font-family: 'Noto Sans Mono', monospace"} - <font size="8">arguments</font>
...
## [ellipses()]{style="font-family: 'Noto Sans Mono', monospace"} - <font size="8">values</font>

```{r, echo = FALSE, eval = TRUE}
out <- biplot (iris[,1:4]) |> PCA(group.aes = iris[,5]) |> ellipses(kappa = 2) 
str(out)
```

## [PCA(),ellipses()]{style="font-family: 'Noto Sans Mono', monospace"} with [plot()]{style="font-family: 'Noto Sans Mono', monospace"}
...
## [PCA(),ellipse()]{style="font-family: 'Noto Sans Mono', monospace"} with [plot()]{style="font-family: 'Noto Sans Mono', monospace"}

```{r, echo = TRUE, eval = TRUE, results = "hide"}
#chaning the degrees of freedom for the kappa value calculation using alpha
biplot (iris[,1:4]) |> PCA(group.aes = iris[,5]) |> ellipses(alpha = 0.95,
  df = 2) |> plot()
```
...
biplot (iris[,1:4]) |> PCA(group.aes = iris[,5]) |> ellipses(kappa = c(2,3,4),
  lty = c(1,2,3)) |> plot()
```

## [PCA(),ellipses(), alpha.bags()]{style="font-family: 'Noto Sans Mono', monospace"} with [plot()]{style="font-family: 'Noto Sans Mono', monospace"}

```{r, echo = TRUE, eval = TRUE, results = "hide"}
#one alpha bag and kappa ellipse per group
biplot (iris[,1:4]) |> PCA(group.aes = iris[,5]) |> ellipses(kappa = 2.45, 
...
In a CVA biplot, the observations belonging to different groups are represented as points. For any sample point ${\bf{x}}$, it is represented in the two-dimensional CVA biplot by the first two columns of ${\bf{x}}'{\bf{M}}$. Observations that are close together in the biplot are similar in their scores on the first two canonical variables, while observations that are far apart are dissimilar.
...
```{r}
biplot(Diabetes[,1:5], classes = Diabetes[,6]) |> CVA() |> axes() |>  plot()
```
...
```{r, echo = TRUE, eval = TRUE}
biplot(Diabetes[,3:5], classes = Diabetes[,6]) |> CVA() |>  axes(tick.label.side
  = c("below","above","below","above")) |> plot()
```
