...
:::::::: {.fragment style="display: flex; justify-content: space-between;"}
::: {style="flex: 1; padding: 10px;"}
**Main Function** - `biplot()`
:::

::: {.fragment style="flex: 1; padding: 10px;"}
...
::: {.fragment style="flex: 1; padding: 10px;"}
**Operations** `prediction()` `interpolate()` `translate()`
`density()` - `fit.measures()` - `classify()` - `alpha.bags()` -
`ellipses()` - `rotate()` - `reflect()` - `zoom()` - `regress()` -
`splines()`
:::

::: {.fragment style="flex: 1; padding: 10px;"}
**Aesthetic Functions** - `samples()` - `axes()` - `newsamples()` -
`newaxes()`
:::
...
row vectors of ${\bf{X}}$ onto the column space of ${\bf{V}}_2$.
...
```{r,echo=FALSE}
bp <- biplot(iris) |> PCA() |> samples(col="blue",opacity=0.5) |> axes(which=NULL) |> plot()
V <- bp$V
