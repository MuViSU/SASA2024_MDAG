...
biplot(state.x77) |> CVA(state.region) |> means(which = c(2,3),label = TRUE) |> 
  plot()
```
:::
...
<<<<<<< HEAD:3_CVA.qmd
class.examp<-biplot(state.x77) |> CVA(state.region) |> classify(col = c("cornflowerblue","darkolivegreen3","darkgoldenrod","aquamarine")) |> plot()
=======
class.examp<-biplot(state.x77) |> CVA(state.region) |> 
  classify(col = c("cornflowerblue","darkolivegreen3","darkgoldenrod","aquamarine"))
class.examp |> plot()
>>>>>>> main:4_CVA.qmd
```
:::
...
```{r Alpha bags}
<<<<<<< HEAD:3_CVA.qmd
ab.examp<-biplot(state.x77) |> CVA(state.region) |> alpha.bags(alpha = c(0.85,0.9,0.95,0.99),lty = c(1,2,3,4)) |> plot()
=======
ab.examp<-biplot(state.x77) |> CVA(state.region) |> 
  alpha.bags(alpha = c(0.85,0.9,0.95,0.99), lty = c(1,2,3,4))
...
<<<<<<< HEAD:3_CVA.qmd
kc.examp<-biplot(state.x77) |> CVA(state.region) |> ellipses(alpha = c(0.85,0.9,0.95,0.99),lwd = c(1,2,3,4)) |> plot()
=======
kc.examp<-biplot(state.x77) |> CVA(state.region) |> 
  ellipses(alpha = c(0.85,0.9,0.95,0.99), lwd = c(1,2,3,4))
...
bp |>  plot(zoom=TRUE)
```
![Using the zoom argument](SASA2024_MDAG/gifs/zoom_plot.gif)
:::
...
```
![3D PCA biplot]("SASA2024_MDAG/gifs/pca3d_plot.gif")
:::
...
```
![3D CVA biplot]("SASA2024_MDAG/gifs/pca3d_plot.gif")
:::
